...
RSI_PERIODO = 7
RSI_BUY_MIN = 45
RSI_BUY_MAX = 55       # Bajado de 60 a 55 - evitar entradas tardias
RSI_SELL_MIN = 45      # Subido de 40 a 45 - mas conservador
RSI_SELL_MAX = 55
RSI_EXTREMO_MIN = 30   # Subido de 25 a 30 - bloqueo mas temprano
RSI_EXTREMO_MAX = 70   # Bajado de 75 a 70 - bloqueo mas temprano
SCORE_MINIMO = 5       # Score minimo para cualquier nivel (antes era 3)
...
def analizar_niveles(df):
    ...
    rango_ema = abs(e8 - e20) / e20 * 100
    if rango_ema < 0.05:
        print(f"  Mercado lateral (EMA spread {rango_ema:.3f}%) - NO OPERAR")
        return None, 0, 0, rsi_now, e8, e20, precio, False

    if buy_score >= sell_score and buy_score >= SCORE_MINIMO:
        direccion = 'BUY'
        score = buy_score
    elif sell_score > buy_score and sell_score >= SCORE_MINIMO:
        direccion = 'SELL'
        score = sell_score
    else:
        return None, 0, 0, rsi_now, e8, e20, precio, False
